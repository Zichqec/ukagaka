...
function makePaginationNav()
{
	for (let i = 0; i < 2; i++)
	{
		...
		if (i == 0) //Top only
		{
			...
			if (pagination == true)
			{
				output += `<br><p class="centertxt" class="paginationPerPageDisplay"><i>(${itemsperpage} per page / ${shown_count} total)</i></p>`;
			}
			else
			{
				output += `<br><p class="centertxt" class="paginationPerPageDisplay"><i>(Showing all ${shown_count} entries)</i></p>`;
			}
		}
